...
(function(win) {
  ...
  if (win._aliqLoadedJS) return;
  else win._aliqLoadedJS = true;
...
/**
 * aliq CSS/JS form helpers module
 * @module lib/forms.py
 */
...
function jqLiteType(somevar) {
  ...
  if (typeStr.indexOf('[object ') === 0) {
    ...
    throw new Error("aliq: Could not understand type: " + typeStr);
  }    
}
...
function jqLiteOn(element, events, callback, useCapture) {
  ...
  var cache = element._aliqEventCache = element._aliqEventCache || {};  
...
var config = require('../config'),
    ...
    scrollLock = 0,
    scrollLockCls = 'aliq-body--scroll-lock',
    scrollLockPos,
...
function raiseErrorFn(msg, useConsole) {
  if (useConsole) {
    if (typeof console !== 'undefined') console.error('aliq Warning: ' + msg);
  } else {
    throw new Error('aliq: ' + msg);
  }
}


/**
 * Register callbacks on aliqNodeInserted event
...
/**
 * Execute aliqNodeInserted callbacks
 * @param {Event} ev - The DOM event.
 */
function animationHandlerFn(ev) {
  // check animation name
  if (ev.animationName !== 'aliq-node-inserted') return;
...
module.exports = {
  ...
  /** Register aliqNodeInserted handler */
...
var jqLite = require('./lib/jqLite'),
    ...
    attrKey = 'data-aliq-toggle',
    attrSelector = '[data-aliq-toggle="dropdown"]',
    openClass = 'aliq--is-open',
    menuClass = 'aliq-dropdown__menu';
...
function initialize(toggleEl) {
  // check flag
  if (toggleEl._aliqDropdown === true) return;
  else toggleEl._aliqDropdown = true;
...
var util = require('./lib/util'),
    ...
    overlayId = 'aliq-overlay',
    bodyClass = 'aliq--overflow-hidden',
...
function overlayOff() {
  ...
  if (overlayEl) {
    ...
    // callback reference
    callbackFn = overlayEl.aliqOptions.onclose;
...
var jqLite = require('./lib/jqLite'),
    util = require('./lib/util'),
    btnClass = 'aliq-btn',
    btnFABClass = 'aliq-btn--fab',
    rippleClass = 'aliq-ripple-effect',
...
function initialize(buttonEl) {
  // check flag
  if (buttonEl._aliqRipple === true) return;
  else buttonEl._aliqRipple = true;
...
function mouseDownHandler(ev) {
  ...
  if (!buttonEl.aliqMouseUp) {
    jqLite.on(buttonEl, mouseUpEvents, mouseUpHandler);
    buttonEl.aliqMouseUp = true;
  }
  ...
  util.requestAnimationFrame(function() {
    jqLite.addClass(rippleEl, 'aliq--animate-in aliq--active');
  });
}
...
function mouseUpHandler(ev) {
  ...
  while (i--) {
    ...
    if (jqLite.hasClass(el, rippleClass)) {
      jqLite.addClass(el, 'aliq--animate-out');
      rippleEls.push(el);
    }
  }
...
var jqLite = require('./lib/jqLite'),
    ...
    wrapperClass = 'aliq-select',
    cssSelector = '.aliq-select > select',
    menuClass = 'aliq-select__menu',
    selectedClass = 'aliq--is-selected',
    doc = document,
    win = window;
...
function initialize(selectEl) {
  // check flag
  if (selectEl._aliqSelect === true) return;
  else selectEl._aliqSelect = true;
...
function Menu(wrapperEl, selectEl) {
  ...
  jqLite.scrollTop(this.menuEl, this.menuEl._aliqScrollTop);
...
Menu.prototype._createMenuEl = function(wrapperEl, selectEl) {
  ...
  menuEl._aliqScrollTop = props.scrollTop;
...
var jqLite = require('./lib/jqLite'),
    ...
    controlsAttrKey = 'data-aliq-controls',
    activeClass = 'aliq--is-active',
    showstartKey = 'aliq.tabs.showstart',
    showendKey = 'aliq.tabs.showend',
    hidestartKey = 'aliq.tabs.hidestart',
    hideendKey = 'aliq.tabs.hideend';
...
function initialize(toggleEl) {
  // check flag
  if (toggleEl._aliqTabs === true) return;
  else toggleEl._aliqTabs = true;
...
/**
 * aliq CSS/JS form-control module
...
var jqLite = require('./lib/jqLite'),
    util = require('./lib/util'),
    cssSelector = '.aliq-textfield > input, .aliq-textfield > textarea',
    emptyClass = 'aliq--is-empty',
    notEmptyClass = 'aliq--is-not-empty',
    dirtyClass = 'aliq--is-dirty',
    floatingLabelClass = 'aliq-textfield--float-label';
...
function initialize(inputEl) {
  // check flag
  if (inputEl._aliqTextfield === true) return;
  else inputEl._aliqTextfield = true;
...
module.exports = {
  ...
  initListeners: function() {
    ...
    setTimeout(function() {
      var css = '.aliq-textfield.aliq-textfield--float-label > label {' + [
        '-webkit-transition',
        '-moz-transition',
        '-o-transition',
        'transition',
